...
public class RoomTest {
	...
	private static void test_viewAllRoom (int t_set, int i_num){
		...
		if(i_num==0){
			...
			test_fillRoom(h_msg, t_set, i_num);
			Room room = new Room();
			Message searchResults = room.getAllRooms(h_msg);
			
			if(h_msg.response.responseCode == ResponseMessage.ResponseCode.SUCCESS){
				System.out.println("Passed Test ID 1");
				System.out.println("Input: " + h_msg.rooms[0].roomNumber);
				System.out.println("Output: # of Rooms: " + searchResults.rooms.length);
				for (int i = 0; i < searchResults.rooms.length; i ++) {
					test_printRoom(searchResults.rooms[i]);
				}
		...
		if(i_num==1){
			...
			if (h_msg.rooms[0].validateParam(h_msg)) {
				searchResults = room.getFilteredRooms(h_msg);
			} else { 
				h_msg.response.responseCode = ResponseMessage.ResponseCode.FAIL;
				h_msg.response.responseString = "invalid inputs";
			}
